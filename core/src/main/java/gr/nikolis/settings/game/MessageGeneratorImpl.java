...
@Slf4j
@Component //this creates the instance (Bean)
public class MessageGeneratorImpl implements MessageGenerator {
    ...
    @Autowired // call auto constructor with game definition
    ...
    public String getResultMessage() {
        if (game.isGameWon()) return getMessage(WIN, game.getNumber());
        else if (game.isGameLost()) return getMessage(LOSE, game.getNumber());
        else if (!game.isValidNumberRange()) {
            String invalid = getMessage(INVALID_RANGE);
            return getMessage(REMAINING, invalid, game.getRemainingGuesses());
        }
        else if (game.getRemainingGuesses() == game.getGuessCount()) return getMessage(FIRST_GUESS);
        else {
